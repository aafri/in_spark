 ...
 */
object RecommendGraphArgbak extends App {
  ...
  val retestActiondate = actioninfolist.map(line => line(4)).distinct().collect()
  val retestPhotodate = actioninfolist.map(line => line(1)).distinct().collect()
  ...
  for (li <- broadPhotoDayList) {
    val actionAfterFilter = actioninfolist.filter(line => line(2) == li)
    ...
    val photoVertex: RDD[(VertexId, Int)] = actionAfterFilter.map(phline => (phline(0).toLong, phline(3).toInt))
