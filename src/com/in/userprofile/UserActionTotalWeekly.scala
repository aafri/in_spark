 ...
 */
object UserActionTotalWeekly {
   def main(args: Array[String]) {
      val conf = new SparkConf().setAppName("UserActionTotalWeekly").setMaster("spark://hadoop70:7077,hadoop73:7077")
      val sc = new SparkContext(conf)
    //0a327b51181ad2139bcf02e8725d0df2,12637248,20140609204518
      val  df = new SimpleDateFormat("yyyyMMdd")
      val currentday= df.format(new Date())
      val initFlag=args(1)
      val lastDay=  TimeUtil.addDateWithoutMLine(args(0),-7)
      val  startDay=if(initFlag =="0") {
        lastDay
      }else{
         args(0)
      }
      val broadcastStartDay=if(initFlag =="0") {
        sc.broadcast(lastDay)
      }else{
         sc.broadcast(startDay)
      }
      val broadCurrentday=sc.broadcast(currentday)
      //0 is init ,1 is normal caculation
    val intervalDay=7
    ...
    val publishPhotoInfo =  GetBetweenDayRdd.getBetweenDayRdd(sc, "/userprofile/photo", startDay, intervalDay)
...
//××××××××××××××××××××××××××××××登录***********************//
    ...
    val userlogin = GetBetweenDayRdd.getBetweenDayRdd(sc, "/userprofile/user_login", startDay, intervalDay)
    ...
    //×××××××××××××××××××发图×××××××××××××××××××××××××××××××××××//
    val photoPaster=GetBetweenDayRdd.getBetweenDayRdd(sc, "/userprofile/photo_paster/", startDay, intervalDay).distinct()
  ...
  // photopublish.saveAsTextFile(args(2))
    ...
    */
    ...
    val userwatchkv13 = userwatchallkv.leftOuterJoin(userwatchIsEathOtherkv).map(line=>(
       line._1,line._2._1,line._2._2.getOrElse(0))).filter(line=>line._2==1 &&line._3==3)
    val userwatchkv00 = userwatchallkv.leftOuterJoin(userwatchIsEathOtherkv).map(line=>(
      line._1,line._2._1,line._2._2.getOrElse(0))).filter(line=>line._2==0 &&line._3==0)
    val userwatchkv=userwatchkv13.union(userwatchkv00).map(line=>(line._1,line._2))
    val photoLove = GetBetweenDayRdd.getBetweenDayRdd(sc, "/userprofile/user_love", startDay, intervalDay)
...
//点赞的人，时间    去重后变成
   ...
   // socailcontact.saveAsTextFile(args(1))
    //××××××××××××××××××××××××××话题××××××××××××××/
    val user_topic=GetBetweenDayRdd.getBetweenDayRdd(sc, "/userprofile/user_browse_topic", startDay, intervalDay)
   ...
   // userTopic.saveAsTextFile(args(1))
    ...
    val allWithLogin=loginJoinSomethingUnionResultWithLogin.union(photopublishWithLogin).union(socailcontactwithLogin).union(userTopicWithLogin).map(
      line=> line._1+","+line._2._1.split(",")(0)+","+line._2._1.split(",")(1)+","+broadcastStartDay.value)
    val allWithLoginKV=loginJoinSomethingUnionResultWithLogin.union(photopublishWithLogin).union(socailcontactwithLogin).union(userTopicWithLogin).map(
      line=> (line._1+","+line._2._1.split(",")(0),line._2._1.split(",")(1)))
    if(initFlag=="0"){
       allWithLogin.saveAsTextFile("/userprofile/useractionresultweekly/"+lastDay)
    }else{
      val lastWeekData =sc.textFile("/userprofile/useractionresultweekly/"+lastDay)
     // 66945439,guang,3
      val lastWeekDataKV=lastWeekData.map(line=>line.split(",")).map(line=> (line(0)+","+line(1),line(2)))
      val twoWeeksDataJoin= lastWeekDataKV.fullOuterJoin(allWithLoginKV).map( { line => val key=line._1
                   val lastWeekValue=line._2._1.getOrElse("0")
                   val thisWeekValue=line._2._2.getOrElse("0")
                   val changeRatio=if(lastWeekValue=="0"){
                       thisWeekValue+".0"
                   }else if(thisWeekValue=="0"){
                     "-"+lastWeekValue+".0"
                  }else {
                     (((thisWeekValue.toInt-lastWeekValue.toInt).toDouble/lastWeekValue.toInt)*100.0/100.0).toString
                  }
                 key+","+thisWeekValue+","+changeRatio+","+broadcastStartDay.value
      })
      twoWeeksDataJoin.saveAsTextFile("/userprofile/useractionresultweekly/"+startDay)
    }
  }
}
