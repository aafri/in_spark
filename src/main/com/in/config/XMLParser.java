...
public class XMLParser {
	 ...
	 */
	public static List<com.in.config.StatUnit> retrieveUnits(String xmlFile) {
		List<com.in.config.StatUnit> groupUnitList = new ArrayList<com.in.config.StatUnit>();
		try{
			...
			for (Iterator<?> statUnitIterator = root.elementIterator("stat_unit");statUnitIterator.hasNext();) {
				...
				com.in.config.StatUnit sUnit = new com.in.config.StatUnit(action);
	 ...
	 */
	public static List<com.in.config.MatchUnit> retrieveFilterUnits(String xmlFile) {
		// TODO Auto-generated method stub
		List<com.in.config.MatchUnit> list = new ArrayList<com.in.config.MatchUnit>();
		try {	
			Element root = XMLParser.getRootElementFromXML(xmlFile);
			for(Object obj1 : root.selectNodes("match")){
				Element matchElement = (Element)obj1;
				com.in.config.MatchUnit match = new com.in.config.MatchUnit();
	 ...
	 *	
	 */
	private static void getGroupItemFromStatUnit(Element unit, com.in.config.StatUnit statUnit){

		List<com.in.config.GroupUnit> groupList = statUnit.getGroupUnit();
		...
		for (Iterator<?> groupIterator = unit.elementIterator("group");groupIterator.hasNext();) { // match level
			...
			com.in.config.GroupUnit groupUnit = new com.in.config.GroupUnit();
	 ...
	 */
	private static void getMatchItemFromStatUnit(Element unit, com.in.config.StatUnit statUnit){
		List<com.in.config.MatchUnit> matchItemList = statUnit.getMatchUnit();
		for (Iterator<?> matchIterator = unit.elementIterator("match");matchIterator.hasNext();) { // match level
			...
			com.in.config.MatchUnit matchItem = new MatchUnit(combineType,groupMatchIndex);
