...
public class AccessLog {
	 ...
	 *            一条Log记录
	 * @param isParma
	 *            如果为true，则保留request部分？后的参数，否则去除。
	...
	public static Map<String, String> parseLog2Map(String input, String logType) {
		...
		String[] respointimel=input.toString().split("\\s+");
		String	respointime="0";
		String str="0";
		if(respointimel.length==0) {
			respointime="0";
		}else {
			 respointime = respointimel[respointimel.length - 1];
		}
		if(sarray[0].length()==0){
			str ="";
		}else if(sarray[0].length()== input.length()){
			str=input;
			System.out.println(input);
		}else {
			str = input.substring(sarray[0].length() + 1, input.length());
		}
		Map<String, String> fieldMap = new HashMap<String, String>();

		if (logType.equals("ACCESSLOG")) {
			Pattern pattern = com.in.sparkrddtest.LogRegex.ACCESSLOG;
			...
			if (m.find()) {
				...
				fieldMap.put(Consnt.LOGITEM_RESPONSE,respointime);
			}
		}
	...
	public static Map<String, String> parseClog2Map(String input, String logType) {
		...
		if (logType.equals("ACCESSLOG")) {
			Pattern pattern = com.in.sparkrddtest.LogRegex.ACCESSLOGTOTAL;
			Matcher m = pattern.matcher(input);
			if (m.find()) {
	...
	public static TrasferedValue genTrasferedValue(Map<String, String> trasferMap,Text value){
