 ...
 */
object ReadRecommendDataInfoPro extends App {
...
val reverScanPhoto=new Scan();
  ...
  reverScanPhoto.setStopRow(Bytes.toBytes(args(0) + "_" + usertoUsertime_stop))
    ...
    var photoList2 = new scala.collection.mutable.ListBuffer[(Long,Long,Long)] //存储由相邻user得到的照片
  ...
  for (duserid <- userListtake) {
          ...
          // photoList1不能包含已看过的照片
           if (!photoList1.contains(keylist(2).toLong)) {
             if(!action_set.contains(keylist(2)+"_"+value1+"_"+keylist(0))) {
               ...
               scanUser.setCaching(100)
               scanUser.addFamily(Bytes.toBytes("f"))
               scanUser.setStartRow(Bytes.toBytes(keylist(2) + "_" + (args(5)+"000").toLong))
               scanUser.setStopRow(Bytes.toBytes(keylist(2) + "_" + (args(6)+"000").toLong))
               val photoHotResultScan = getUserFromPhotohot_table.getScanner(scanUser)
               //var result1=new Result()
               var userResult = new Result()
               val photoHotResultScani = photoHotResultScan.iterator()
               var zan=0l
               while (photoHotResultScani.hasNext) {
                 val result1 = photoHotResultScani.next()
                 if (result1 != null) {
                   while (result1.advance()) {
                     val result1Cess = result1.getValue( Bytes.toBytes("f"),   Bytes.toBytes("h"))
                     // val cellqulifiers = result1.get.getColumnCells()
                     // for (cellq <- cellqulifiers) {
                     val value = Bytes.toString(result1Cess)
                   //  println()
                   //  println("value:"+value)
                     zan=zan+value.toLong
                   }
                 }
               }
              // photoList2.append((keylist(2).toLong,(qua + duserid._2)*(1+ (zan*pIndex/100).toInt), qua + duserid._2,zan))
               //照片的关系和view,love,
               if(zan >= actionPowerMap.getOrElseUpdate(22, 0)) {
                 photoList2.append((keylist(2).toLong, qua + duserid._2, zan))
               }
               //println(keylist(2).toLong +"zan:"+zan)
               photoHotResultScan.close()
             }
           }
         }
       }
    ...
    var usercountMap2 = new mutable.HashMap[Long, Long]
  var usercountMap3 = new mutable.HashMap[Long, String]

  //排序后计算照片权值
  val pIndex=actionPowerMap.getOrElseUpdate(21, 0)
  ...
  if(sortPhotoList2.length!=0){
    ...
    m=sortPhotoList2(0)._2
      n=sortPhotoList2(0)._3
    ...
    // val currv3=li._4
      if (pre1.equals(curr1)) {
         ...
         usercountMap2.put(pre1, (m*(1+ (n*pIndex/100))).toLong)
           usercountMap3.put(pre1,m+","+n)
         m =currv1
         n=currv2
       //  k=currv3
       }
  ...
  var forColdUserPSet = new scala.collection.mutable.HashSet[Long]
  var get:Get=null
      ...
      if (stringvalue != null && stringvalue != "") {
        if (args(5).toLong <= stringvalue.toLong && args(6).toLong >= stringvalue.toLong) {
          if (!photoList1.contains(line._1)) {
            println(line._1 + ","+line._2+","+usercountMap3.getOrElse(line._1,","))
          }
         }
        }
      }
