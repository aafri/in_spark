...
trait SparkHadoopMapReduceUtilExtended  {
  ...
  def newTaskAttemptID(
    ...
    try {
      ...
      ctor.newInstance(jtIdentifier, new Integer(jobId), new java.lang.Boolean(isMap), new Integer(taskId),
        new Integer(attemptId)).asInstanceOf[TaskAttemptID]
    } catch {
      case exc: NoSuchMethodException => {
        ...
        val ctor = klass.getDeclaredConstructor(classOf[String], classOf[Int], taskTypeClass,
          classOf[Int], classOf[Int])
        ctor.newInstance(jtIdentifier, new Integer(jobId), taskType, new Integer(taskId),
          new Integer(attemptId)).asInstanceOf[TaskAttemptID]
      }
    }
  }
